...
import { Bell, LogOut, Settings, User, Search, ChevronLeft } from "lucide-react";
...
export function Topbar({ userEmail, userInitials, alertCount = 0 }: TopbarProps) {
  ...
  // Show a back button on inner pages (mobile only) — dashboard is the home, no back from there
  const showBack = pathname !== "/dashboard" && pathname !== "/onboarding";
  ...
  return (
    <header
      className="sticky top-0 z-30 glass-topbar pwa-safe-top"
      role="banner"
    >
      <div className="flex h-14 items-center gap-2 px-4 lg:px-6 lg:gap-3">
